...
## 8. Addendum — second verification pass (2026-09-10)

*A parallel SEO session re-rendered every public route from the repository after the
review round. It confirms §5–§6 and adds only what they lack. The objection above is
answered: the brief has struck "or disclose it" (§8.1, §13.5). Labels as in the brief:
pre-condition (m01), copy only, after the freeze.*

1. **Correction to §6, robots and sitemap.** With the admin gate on, as production runs
   it (`CHORDENTIAL_ADMIN_TOKEN` set), `/robots.txt`, `/sitemap.xml`, `/about` and
   **every unknown path** answer 303 to `/admin/login`, which returns 200 with the
   *Procurement OS* login page (Brand Strategist 17; brief §8.4). "Both 404" holds only
   with the gate off. No URL on this site ever 404s to a crawler; how Google reads a
   robots file that redirects to HTML is *(verify)*. *After the freeze*: the two files
   in `publicpaths._PUBLIC_PATHS`, and a plain 404 for unknown paths outside the gate.
   The login page's title is *copy only* now.
2. **`/pricing`** is `console_routes.py:914`, behind the gate (verified: 303 to login).
   P6 never links it; under §13.10 P6 and D4 carry table and verdict, no figure.
3. **The audio swap under §13.5 has two file sets.** `listening_rows()` serves the
   45-second excerpts `ex-demo-*.mp3` via `excerpts.json`, not the masters; all four
   served masters carry the ID3 tag *Generated by ACE Studio* (read from the files).
   Replacing the masters alone leaves the front door playing the old excerpts.
4. **`/reel`** is a 307 (`public.py:213`); make it a 301. **`/thanks`** is indexable;
   `noindex` it. Both *after the freeze*.
5. **Weight, measured.** `/` is 33 KB of HTML, 23 KB of it inline CSS, and ships the
   wordmark as a 43 KB PNG at 2558×660 px for a 38 px mark; `SelectiveGZip` compresses
   text only. An SVG wordmark, *after the freeze*.
6. **§13.8 touches the `/capabilities` rewrite above:** a craft is listed only when a
   signed creator does it, so the five H2s render from the signed roster, not from
   `showcase.CAPABILITIES`. *Copy only* once the roster is ruled.
