...
*Wave 4, 2026-09-10. Reads under `docs/marketing/00-brief.md`; the brief wins. Every term
is READ OUT of `composer_agreement.py`, `service_agreement.py`, `compensation.py`,
`agreements.py`, `talent.py`, `matching.py`, `web/room.py`, `web/creator_routes.py` —
nothing is invented. Revenue is zero, no roster is signed (brief §13.8), and nothing here
may tell a composer that work is waiting.*

**Fix before this is used.** `recruiting.py` says *"real, paid briefs (never spec)"* and
`for_artists.html` says *"We bring you scoped, paid work"* — present tense, zero
engagements sold. Honest form: *when a brief fits, you hear first, and it will be paid.*
...
- **Licence income is shared too:** if the client later pays to extend term, widen
  ...
  net of that payment, for as long as the work earns**, surviving the agreement's end. **A
  demo we request that loses is paid $400 flat** (`DEMO_FEE`).
...
- **Cue sheet:** *"The studio credits the writer as composer on the cue sheet it files,
  ...
  A client's own credit list is a **promise to ask**, not a promise it happens.
...
- **Their risk:** they warrant the work is human-made and clean; liability per engagement
  is **capped at the greater of $25,000 and 3× the fees paid them for it**. Florida law,
  Miami-Dade courts (defaults).
...
Delivery targets are named per medium so nobody guesses — 48 kHz / 24-bit WAV, US
broadcast −24 LKFS ±2 (ATSC A/85), European −23 LUFS ±1 (EBU R128), streaming −14 LUFS.
Same acceptance window, kill fee, backstop, cap and credit promise.
...
Nothing signed until it means something (§5).
...
recorded discipline, so a count means *approved and described*.

| Profile (code discipline) | The work | Evidence that qualifies | Need |
|---|---|---|---|
| **Spot composer, to picture** (`COMPOSITION`) | :60/:30/:15 to a near-locked cut; hits and a button on the logo; stems and cutdowns from one session | 2+ released spots or brand films where they wrote the cue; a reel of music *against picture*, not tracks | **3** — never one. One composer is a single point of failure on a date tied to an air date |
| **Sonic identity writer** (`SONIC_BRANDING`) | A mnemonic that survives 10,000 plays, and its variants | A sound logo you can hear in the wild, or a mnemonic system with its family | **1**, may be the same person |
| **Arranger / orchestrator** (`ARRANGEMENT`) | Charts, session prep, conducting; making a mock-up sound played | Charts they wrote; a session prepped or conducted | **1**. Also the 40% share, and the bridge to the room Jon wants |
| **Mix / master engineer** (`MIXING`) | Mix from the composer's approved stems to the medium's loudness target | Talks about −24 LKFS / ATSC A/85 without looking it up; broadcast credits; a TV mix | **1** |
| **Music editor / versioning** (`MIXING`) | Cutdowns, verticals, conforms to a re-cut picture | A job where the picture changed and the music followed; EDLs, fps, start timecode | **1**, usually the mixer |
| **Sound designer** (`SOUND_DESIGN`) | Designed elements under and around the cue | A design reel; can say whether a library licence permits stem delivery | **1** |
| **Supervisor / clearance researcher** (`SUPERVISION`) | Reads other people's paper; could help staff the Clearance Read | Clearance research done; a cue sheet filed | **0–1**, not needed to deliver |

**Five to seven people.** Jon's stated gap — "none in the style I'd hire" — is profile
one, so that is where the evening in §6 goes.
...
designate. **No money in public, ever** (brief §7). **Never post a brief that does not
exist.** One post per room, then answer replies — reading other people's threads comes
first.
...
| **Society of Composers & Lyricists** (thescl.com) | Media composers; confirmed to consider *"music creators for commercials … case by case."* The credentialled end of profile 1 | Join as the studio side, attend before asking *(verify which category fits a studio operator)*. A studio that publishes its terms is unusual there |
| **Airwiggles** (airwiggles.com), free | Sound designers; 8,000+ members, runs the Audio Challenge and Noisevember. Profile 6, with public dated work | Contribute to a challenge first. A craft room, not a job board *(verify whether a work post is welcome at all)* |
...
| **Reddit** — r/composer, r/WeAreTheMusicMakers, r/audioengineering, r/GameAudio *(verify each)* | Working and emerging composers and engineers; public reel threads | Hard self-promo rules everywhere; message the mods **before** posting. We bring straight answers on rights |
...
one local and tied to the room he wants.
...
> each one as a conform, a revision or out of scope first, and the out-of-scope ones stop at
> me. Rounds are counted. The client never gets your name or your email. And you're paid
> within 120 days of the client accepting the work, whether or not the client has paid me.
>
...
> creative fee rather than a flat buyout, half the publisher's share to the writers, your
> writer's share never touched, your name on every cue sheet I file, and payment within 120
> days of the client accepting whether or not the client has paid me.
>
...
location, reel URL, `review_status`, `invite_status`, source, rate and unit.
`talent.matchable` is true only when Jon has **approved the reel** and **one discipline is
recorded**. `matching.match_talent` then ranks approved creators against a brief — 60%
discipline fit, 30% overlap between the brief's words and their credits, 10% profile
completeness — and **explains every score**. It never assigns; assignment is Jon's button.
...
exist before assignment, which is *after* a real job exists. Nothing requires one at roster
time.
...
2. **Signature, when a brief has their name against it**, with scope and fee already in
   writing, so document and job arrive together.
3. **Engagement acceptance**, separately, per job, as clauses 2 and 3A require anyway.

**What signing does not mean:** work is not coming; it does not stop them working for
anyone else; it does not touch their writer's share.
...
`source_url` from where it was found; a name with no public evidence beside it does not
enter — ADR-0050's rule applied to supply.
...
The demand-side plan (`roadmap.md` §03) stays. Supply takes **four hours a week out of the
...
rate, and `/capabilities` lists no craft until a signed creator does it. So the roster must
be *reachable* by the time a proposal is countersigned. Reachable, not signed.

| Week | Supply (≈4h) | Must not displace |
|---|---|---|
| **1** | Fix the two copy faults (30 min). Build the ten rows of §6 (2h). Read the agreement end to end as a composer would, writing down every question he can't answer (1h) | m01's front door; the first five sequences |
| **2** | Message one to slots 1–5, three a day, same-day replies (2h). Join and read VI-Control, post nothing (1h). Email Frost the alumni question (30 min) | m03's posts; the next five sequences |
| **3** | Message one to slots 6–10. Two calls with anyone who replied — the questions are his to ask (2h). Record disciplines, credits and rate on every warm row so `matchable` and `kind_for` resolve (1h) | The Reads. A Read requested beats a composer sourced |
| **4** | One Recording Academy Florida or AFM Local 655 event, in person (3h). Second referral pass: one name from every warm row (30 min) | m02's proof surfaces |
...
four: **six to eight approved rows with rates on file and zero signatures** — what the code
wants, and what he can say out loud without one sentence that is not yet true.

### Objection to the brief

None to the brief; one to two shipped surfaces, recorded at the top. That is the honesty
rule failing on the supply side the way §8 records it failing on the demand side, and it
should be corrected before message one goes to anybody.
...
Reddit communities and their rules; VI-Control's jobs-section rules; whether Airwiggles
permits a work post; which SCL category a studio operator fits; non-alumnus access to
Berklee's alumni tools; Full Sail and SCAD Atlanta programs; which trade outlets publish
