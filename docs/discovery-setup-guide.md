...
| **Transcript never completes** | This was the default's fault until 2026-08-13: `meeting_captions` transcribes the MEETING PLATFORM's closed captions, so it produces nothing at all when the Zoom host has captions switched off — the bot still joins, records and finishes, and no notes ever arrive. The default is now `recallai_streaming` (Recall's own ASR: no third-party key, no dependency on anyone's Zoom settings). A call already recorded without a transcript is automatically sent for async transcription on the next poll, so older meetings recover by themselves. Override with `CHORDENTIAL_RECALL_TRANSCRIPT_PROVIDER` only if you have a reason. |
