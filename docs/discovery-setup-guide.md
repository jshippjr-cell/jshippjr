...
| **Transcript never completes / bot joined but no notes** | Recall's **Logs** will say `Zoom failed to enable captions for meeting, transcript will not be available`. The default provider `meeting_captions` does not transcribe audio — it READS Zoom's caption stream, and the bot joins as a participant, so Zoom refuses to switch captions on for it. Confirmed live 2026-08-13. Fix: set `CHORDENTIAL_RECALL_TRANSCRIPT_PROVIDER=recallai_streaming` (Recall's own speech-to-text — no third-party key, no dependency on the host's Zoom settings), or make the bot a co-host, or turn captions on before the call. A call already recorded this way is sent for async transcription automatically on the next poll. |
