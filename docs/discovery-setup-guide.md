...
| **Transcript never completes / bot joined but no notes** | Recall's **Logs** say `Zoom failed to enable captions for meeting, transcript will not be available`. This was the old default: `meeting_captions` does not transcribe audio, it READS Zoom's caption stream, and Zoom refuses to enable captions for a bot that is not the host. **Fixed in code on 2026-08-13** — the default is now `recallai_streaming` (Recall's own speech-to-text; no third-party key, no dependency on anyone's Zoom settings), so no configuration is required. Calls recorded under the old default are sent for async transcription automatically on the next poll. |
