...
# The demo recordings currently on this site are AI-GENERATED PLACEHOLDERS,
# standing in until real sessions are recorded. Their ID3 artist tag says so —
# every served demo carries TPE1 "Generated by ACE Studio" — and a producer can
# read that with a right-click in any player or a drag into a DAW.
#
# So the page says it before they find it. The alternative, stripping a tag that
# happens to be TRUE so a claim survives, is falsification, and it is the exact
# move the honesty rule exists to prevent.
#
# This is one string rendered on every surface that plays a demo. When real
# sessions are recorded and the placeholders are replaced, this constant and the
# tests that assert it come out together — not one without the other.
PLACEHOLDER_AUDIO_NOTICE = (
    "These recordings are AI-generated placeholders, standing in until we record "
    "the real sessions. They demonstrate the brief and the approach, not our "
    "playing. Music delivered on a commission is written and recorded by people — "
