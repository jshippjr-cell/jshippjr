...
# work and the buyer, never with "new studio" / "no clients": a demonstration is
# the purest proof for a craft house, and saying so is both confident and true.
#
# Until 2026-09-10 every recording on this site was an AI-generated placeholder,
# disclosed by a PLACEHOLDER_AUDIO_NOTICE printed beside every player. The
# founder's own music replaced them, and the notice and the tests that asserted
# it came out together, as they were written to. The rule that survives is the
# brief's §7: no machine-made music, anywhere, including as a demonstration —
# and `tests/test_hear_the_work.py` now fails the build if a served recording's
# own metadata says it was generated.
...
DEMOS_INTRO = {
    ...
    "lede": (
        "What is here is our own writing, not a client's commission. Press play "
        "for the sound; send a brief to hear how we would score yours."
    ),
...
# about the same track. Truthful: a demonstration of the writing, not a client
# commission. The recording is the founder's own; the copy does not name him on the
# player — the brief's rule is to state authorship once, on the About, and never
# wave it — and it does not describe the music, because a description written by
# someone who has not heard it would be exactly the kind of claim the honesty rule
# forbids. The four AI-generated placeholders this list used to carry (each of
# which answered a brief we had set ourselves) came out on 2026-09-10.
#
# Audio is served from our own /static/public/ rather than a third-party CDN: the
# front door should not depend on an external host to make a sound. Replacing or
# adding a track is a file swap at the path below plus
# `python scripts/build_demo_excerpts.py` for its 45-second excerpt — nothing else
# in the app needs to change; the front door lights one note per entry.
