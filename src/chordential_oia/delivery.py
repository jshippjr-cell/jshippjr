...
# --------------------------------------------------------------------------- #
# WHO OWES WHICH DELIVERABLE, and in what order (ADR-0075)
# --------------------------------------------------------------------------- #
...
#
# That is a CHAIN, not three parallel piles: the mixer works from the composer's approved
# master, and the editor works from the mixer's finished mix. It lives here rather than in
# a template because it is a fact about how the studio makes records, and three surfaces
# need the same answer (ADR-0002).
COMPOSER, MIXER, EDITOR = "composer", "mixer", "editor"

#: Which craft a deliverable GROUP belongs to. Asset-level overrides below.
_GROUP_OWNER = {
    "masters": MIXER,               # the derivative masters: instrumental, TV mix, VO-safe
    "cutdowns": EDITOR,
    "social verticals": EDITOR,
    "production assets": COMPOSER,  # the stem package is the composer's bounce
    "documentation": "",            # the studio's paperwork, nobody's upload
}
#: Assets whose owner is not their group's.
_ASSET_OWNER = {
    "final master": COMPOSER,       # the version ladder itself — not a lane
}
#: What each craft waits on. Absent = ready as soon as the master is approved.
_OWED_AFTER = {EDITOR: MIXER}
#: The words an assignment role can be written with, longest first so "music editor"
#: is not read as "editor" by accident when both would match.
_ROLE_WORDS = (
    (COMPOSER, ("composer", "composition", "writer", "songwriter", "arranger")),
    (MIXER, ("mix engineer", "mixing engineer", "audio engineer", "mixer", "mixing",
             "engineer", "mastering")),
    (EDITOR, ("music editor", "picture editor", "editor", "edit")),
)


def role_key(role: str) -> str:
    """The craft behind an assignment's role string ("" when it is none of them).

    Assignment roles are free text — "Mixer", "Audio Engineer", "Mix engineer" are one
    craft, and the room has to know that before it can say whose lane a row is.
    """
...
def deliverable_owner(asset: str, group: str = "") -> str:
    """Which craft owes this deliverable ("" = the studio's, or unknown)."""
    a = (asset or "").strip().lower()
    if a in _ASSET_OWNER:
        return _ASSET_OWNER[a]
