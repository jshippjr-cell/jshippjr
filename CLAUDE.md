...
- **`app.py` is being taken apart** (ADR-0044) — it was 9,133 lines and is now 2,004. Below it sit
  ...
  `campaign_routes.py`, `simulator_routes.py`) and the helper layer
...
- Test: `python -m pytest tests/ -q` (runs **parallel via pytest-xdist `-n auto`**,
  ...
  in batches of ~7 files with `-n0` instead. **1,385 tests**, must stay green before
  commit.
