...
unchanged). When ON, internal routes require the cookie; the public /site, static,
health, and login routes stay open.
"""
...
def test_gate_off_by_default(tmp_path, monkeypatch):
    ...
    with TestClient(app_mod.app) as c:
        ...
        assert c.get("/site").status_code == 200
...
def test_gate_on_public_surfaces_stay_open(tmp_path, monkeypatch):
    ...
    with TestClient(app_mod.app) as c:
        assert c.get("/site").status_code == 200
        assert c.get("/site/start").status_code == 200
        ...
        assert c.post("/site/start", data={"contact_name": "X"},
...
def test_wrong_passphrase_denied(tmp_path, monkeypatch):
    app_mod = _build(tmp_path, monkeypatch, gated=True)
    with TestClient(app_mod.app) as c:
        r = c.post("/admin/login", data={"password": "nope", "next": "/"},
...
def test_correct_passphrase_grants_access(tmp_path, monkeypatch):
    app_mod = _build(tmp_path, monkeypatch, gated=True)
    with TestClient(app_mod.app) as c:
        r = c.post("/admin/login", data={"password": TOKEN, "next": "/"},
                   follow_redirects=False)
        assert r.status_code == 303
...
def test_logout_revokes_access(tmp_path, monkeypatch):
    app_mod = _build(tmp_path, monkeypatch, gated=True)
    with TestClient(app_mod.app) as c:
        c.post("/admin/login", data={"password": TOKEN, "next": "/"})
        assert c.get("/", follow_redirects=False).status_code == 200
        c.get("/admin/logout")
        assert c.get("/", follow_redirects=False).status_code == 303
