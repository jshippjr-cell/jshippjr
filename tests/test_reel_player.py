...
def test_active_card_pops_forward_and_others_dim(client):
    ...
    assert "translateZ(calc(var(--radius,300px) + 90px)) scale(1.06)" in css
    assert ".rg-drum.rg-has-active .rg-card:not(.rg-active){filter:brightness(.4) saturate(.75)}" in css


def test_list_mode_and_narrow_screen_clear_the_dimming_even_with_an_active_card(client):
    """The dimming rule has HIGHER CSS specificity than a plain `filter:none`
    reset, so list mode / the narrow-screen fallback each need their own
    exact-shape override or a dimmed card would stay dimmed forever once
    .rg-has-active is set (that class isn't cleared by the JS-free view
    toggle)."""
    css = _reel_css()
    assert ".rg-view-input:checked ~ .rg-stage .rg-drum.rg-has-active .rg-card:not(.rg-active){filter:none}" in css
    assert css.count(".rg-drum.rg-has-active .rg-card:not(.rg-active){filter:none}") >= 1
...
def test_carousel_js_defaults_to_the_first_track_looping_on_sound_entry(client):
    ...
    assert "loadTrack(trackCards[0], true);" in js
